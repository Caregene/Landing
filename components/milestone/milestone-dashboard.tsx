...
export function MilestoneDashboard({ selectedChildId }: { selectedChildId?: string }) {
  ...
  const recentActivities = [
    {
      id: "1",
      type: "milestone",
      title: "Completed 18-month milestone assessment",
      description: "Achieved 12 out of 15 milestones",
    ...
    },
    {
      id: "2",
      type: "appointment",
      title: "Pediatric checkup scheduled",
      description: "Dr. Smith - March 15, 2024 at 10:00 AM",
    ...
    },
    {
      id: "3",
      type: "activity",
      title: "New developmental tip viewed",
      description: "Language development activities for 18 months",
    ...
    },
    {
      ...
      description: "First words milestone completed",
  ...
  const handleExportData = (format: "pdf" | "json" | "csv") => {
    ...
    const exportData = {
      ...
      milestoneProgress: {
        currentAge: childData.currentAgeKey,
        totalItems: childData.totalItems,
        answeredItems: childData.answeredItems,
        completedItems: childData.completedItems,
        categoryProgress: childData.categoryProgress,
      },
      appointments: upcomingAppointments,
      exportDate: new Date().toISOString(),
    }

    const filename = `${selectedChild.firstName}-milestone-report-${new Date().toISOString().split("T")[0]}`
    ...
    if (format === "json") {
      ...
      // Simple CSV export for milestone progress
      const csvContent = [
        "Category,Completed,Total,Progress",
        ...Object.entries(childData.categoryProgress).map(
          ([category, progress]) =>
            `${category},${progress.completed},${progress.total},${Math.round((progress.completed / progress.total) * 100)}%`,
        ),
      ].join("\n")
  ...
  const handleShareData = (method: "email" | "copy" | "print") => {
    if (!selectedChild) return

    const shareText = `${selectedChild.firstName}'s Milestone Progress Report
    
Current Age: ${childData.currentAgeKey.toUpperCase()}
Progress: ${childData.answeredItems}/${childData.totalItems} milestones reviewed
Completed: ${childData.completedItems} milestones achieved

Category Progress:
${Object.entries(childData.categoryProgress)
  .map(
    ([category, progress]) =>
      `• ${category}: ${progress.completed}/${progress.total} (${Math.round((progress.completed / progress.total) * 100)}%)`,
  )
...
Generated on: ${new Date().toLocaleDateString()}`
    ...
    if (method === "email") {
      const subject = encodeURIComponent(`${selectedChild.firstName}'s Milestone Progress Report`)
      ...
      if (printWindow) {
        printWindow.document.write(`
          <html>
            <head><title>${selectedChild.firstName}'s Milestone Report</title></head>
            <body style="font-family: Arial, sans-serif; padding: 20px;">
              <pre style="white-space: pre-wrap;">${shareText}</pre>
            </body>
          </html>
        `)
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="p-5 bg-teal-50 rounded-xl border border-teal-200">
              <h4 className="font-semibold text-teal-800 mb-2">Social & Emotional Development</h4>
              <p className="text-teal-700 leading-relaxed">
                Encourage pretend play and help your child express emotions through words and gestures.
              </p>
            </div>
            <div className="p-5 bg-blue-50 rounded-xl border border-blue-200">
              <h4 className="font-semibold text-blue-800 mb-2">Language & Communication</h4>
              <p className="text-blue-700 leading-relaxed">
                Read together daily and ask simple questions about pictures in books to build vocabulary.
              </p>
            </div>
            <div className="p-5 bg-teal-50 rounded-xl border border-teal-200">
              <h4 className="font-semibold text-teal-800 mb-2">Cognitive Development</h4>
              <p className="text-teal-700 leading-relaxed">
      ...
      <Card className="border-gray-200 shadow-sm rounded-xl">
        <CardHeader className="pb-4">
          ...
          <CardDescription className="text-gray-600">
            Latest milestone progress and healthcare activities
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <Button
              variant="outline"
              size="sm"
              className="w-full border-gray-200 rounded-lg hover:bg-gray-50 bg-transparent"
            >
      ...
      <Card className="border-gray-200 shadow-sm rounded-xl">
        <CardHeader className="pb-4">
          ...
          <CardDescription className="text-gray-600">
            Export milestone data for healthcare provider visits or personal records
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <Button
                  ...
                  className="justify-start border-gray-200 rounded-lg hover:bg-gray-50"
                >
            ...
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
              <h4 className="font-medium text-blue-900 mb-2">Healthcare Provider Integration</h4>
              <p className="text-sm text-blue-800 mb-3">
                Share milestone progress directly with your child's healthcare team for more informed visits.
              </p>
              <Button
                variant="outline"
                size="sm"
