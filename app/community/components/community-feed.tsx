  ...
  const handleReaction = async (postId: string, reactionType: string) => {
    try {
      ...
      if (isRemoving) {
        ...
        // Single reaction supported; backend expects 'love' for posts
              ...
              <div className="flex-1 mb-4 lg:mb-0">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-white/80">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {posts.reduce((sum: number, post: Post) => sum + (post.stats?.totalComments || 0), 0)} comments
                  </span>
                  <span className="flex items-center gap-1">
                    <Heart className="h-4 w-4" />
                    {posts.reduce((sum: number, post: Post) => sum + (post.stats?.totalReactions || 0), 0)} reactions
                  </span>
                </div>
              </div>
            </div>
                  ...
                  {post.images && post.images.length > 0 && (
                    <div className="relative">
                      <div className={`${post.images.length === 1 ? '' : 'grid grid-cols-2 gap-0.5'}`}>
                        {post.images.filter(Boolean).slice(0, 4).map((image: string, index: number) => (
                          <div 
                            key={index} 
