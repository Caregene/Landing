...
export const CommunityManagement: React.FC<CommunityManagementProps> = ({
  ...
  const { communities: groups, loading: groupsLoading, error: groupsError, refetch: refetchGroups } = useCommunities({ page: 1, limit: 8, search: groupSearch || undefined, sort: 'recent' })
  ...
  const handleCreateCommunity = async (communityData: any) => {
    try {
      ...
      if (result.data) {
  refetchUserCommunities()
  // also refresh groups tab list
  try { await refetchGroups() } catch {}
  ...
  return (
    <div>
      <div className="flex items-center justify-between px-3 sm:px-0 py-3 sm:py-4">
      ...
      <div className="px-0 sm:px-0 py-3 sm:py-6">
    ...
    <div className="relative p-2 sm:p-3">
      ...
      >
        ...
        <TabsTrigger
          value="groups"
          className="group inline-flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold transition-all duration-300 
          data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-500 data-[state=active]:to-blue-600 data-[state=active]:text-white data-[state=active]:shadow-inner
          hover:bg-gradient-to-r hover:from-indigo-50 hover:to-blue-100 text-gray-700 hover:text-indigo-700 min-h-[2.75rem] sm:min-h-[3rem]"
        >
          <Users className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
          <span className="text-xs sm:text-sm font-medium">Support Groups</span>
        </TabsTrigger>
        ...
        >
          ...
          <div className="w-full min-h-[50vh]">
            ...
            <TabsContent value="groups" className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 md:grid-cols-2">
                {groupsLoading && (
                  <div className="col-span-full">
                    <Card>
                      <CardContent className="p-6 text-sm text-muted-foreground">Loading groups…</CardContent>
                    </Card>
                  </div>
                )}
                {groupsError && (
                  <div className="col-span-full">
                    <Card>
                      <CardContent className="p-6 text-sm text-destructive">Failed to load groups.</CardContent>
                    </Card>
                  </div>
                )}
                {!groupsLoading && !groupsError && (groups?.length || 0) === 0 && (
                  <div className="col-span-full">
                    <Card>
                      <CardContent className="p-6 text-sm text-muted-foreground">No groups found.</CardContent>
                    </Card>
                  </div>
                )}
                {(groups || []).map((group: any, index: number) => {
                  const activity = (group.posts || 0) >= 300 ? 'Very Active' : (group.posts || 0) >= 100 ? 'Active' : 'Moderate'
                  const isPrivate = !!group.isPrivate
                  const members = group.memberCount ?? 0
                  return (
                    <Card key={group._id || index} className="hover:shadow-md transition-shadow">
                      <CardHeader>
                        <div className="flex items-center justify-between">
                          <CardTitle className="text-lg">{group.title || group.name}</CardTitle>
                      ...
                      <CardContent>
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                            ...
                            <Badge
                              variant={activity === 'Very Active' ? 'default' : 'secondary'}
                              className="text-xs"
                            >
                              {activity}
                            </Badge>
                        ...
                        <Button className="w-full" onClick={async () => {
                          try {
                            const res = await joinCommunity(group._id || group.id)
                            if (res.data) {
                              refetchUserCommunities()
                            }
                          } catch {}
