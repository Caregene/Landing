    ...
    this.children = [
      {
        id: "child-1",
        firstName: "Ava",
        birthDate: "2017-03-15", // Updated to match 7-year-old Ava from health plan
        sex: "F",
...
let milestoneStoreInstance: MilestoneStore | null = null

export const getMilestoneStore = (): MilestoneStore => {
  if (typeof window === "undefined") {
    return {
      getChildren: () => [],
      getChild: () => null,
      createChild: () => ({ id: "", firstName: "", birthDate: "" }),
      updateChild: () => null,
      deleteChild: () => false,
      getChecklist: () => null,
      ...
      getTips: () => [],
      toggleBookmarkTip: () => false,
      getAppointments: () => [],
      createAppointment: () => ({ id: "", childId: "", type: "well visit", dateTime: "", location: "" }),
      updateAppointment: () => null,
      deleteAppointment: () => false,
      getReminderPrefs: () => null,
      saveReminderPrefs: () => {},
      generateSummary: () => null,
      getAgeKeyForChild: () => "2m",
      calculateAge: () => ({ chronologicalAge: 0, correctedAge: 0, ageKey: "2m" }),
    } as any
  }

  if (!milestoneStoreInstance) {
    milestoneStoreInstance = new MilestoneStore()
  }
  return milestoneStoreInstance
}

export const milestoneStore = getMilestoneStore()

export const addChild = (childData: Omit<Child, "id">) => getMilestoneStore().createChild(childData)
export const getChecklist = (ageKey: AgeKey) => getMilestoneStore().getChecklist(ageKey)
export const getChildren = () => getMilestoneStore().getChildren()
export const createChild = (childData: Omit<Child, "id">) => getMilestoneStore().createChild(childData)
export const updateChild = (childId: string, updates: Partial<Child>) =>
  getMilestoneStore().updateChild(childId, updates)
export const deleteChild = (childId: string) => getMilestoneStore().deleteChild(childId)
export const saveChecklistResponse = (response: ChecklistResponse) =>
  getMilestoneStore().saveChecklistResponse(response)
export const getTips = (ageKey?: AgeKey) => getMilestoneStore().getTips(ageKey)
export const toggleBookmarkTip = (tipId: string) => getMilestoneStore().toggleBookmarkTip(tipId)
export const getAppointments = (childId?: string) => getMilestoneStore().getAppointments(childId)
export const createAppointment = (appointmentData: Omit<Appointment, "id">) =>
  getMilestoneStore().createAppointment(appointmentData)
export const updateAppointment = (appointmentId: string, updates: Partial<Appointment>) =>
  getMilestoneStore().updateAppointment(appointmentId, updates)
export const deleteAppointment = (appointmentId: string) => getMilestoneStore().deleteAppointment(appointmentId)
export const getReminderPrefs = (childId: string) => getMilestoneStore().getReminderPrefs(childId)
export const saveReminderPrefs = (prefs: ReminderPreference) => getMilestoneStore().saveReminderPrefs(prefs)
export const generateSummary = (childId: string, ageKey: AgeKey) => getMilestoneStore().generateSummary(childId, ageKey)
export const getChild = (childId: string) => getMilestoneStore().getChild(childId)
export const getChecklistResponses = (childId: string) => getMilestoneStore().getChecklistResponses(childId)
export const getAgeKeyForChild = (childId: string) => getMilestoneStore().getAgeKeyForChild(childId)
export const calculateAge = (birthDate: string, dueDate?: string) =>
  getMilestoneStore().calculateAge(birthDate, dueDate)
